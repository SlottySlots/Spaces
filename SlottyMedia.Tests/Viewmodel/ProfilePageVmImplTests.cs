...
public class ProfilePageVmImplTests
{
    ...
    public async Task Initialize_LoadsUserInfoAndPosts()
    {
        ...
        authServiceMock.Setup(a => a.GetCurrentSession())
            .Returns(new Session { User = new User { Id = userId.ToString() } });
        postServiceMock.Setup(p => p.GetPostsByUserId(It.IsAny<Guid>(), It.IsAny<PageRequest>()))
            .ReturnsAsync(PageImpl<PostDto>.Empty());
        userServiceMock.Setup(u => u.GetUserInfo(It.IsAny<Guid>(), It.IsAny<bool>(), It.IsAny<bool>())).ReturnsAsync(
            new UserInformationDto { UserId = userId, Username = "testuser", FriendsAmount = 5, SpacesAmount = 3 });
    ...
    public async Task LoadPosts_LoadsPostsForUser()
    {
        ...
        userServiceMock.Setup(u => u.GetUserDaoById(It.IsAny<Guid>()))
            .ReturnsAsync(new UserDao { UserId = userId, UserName = "testuser" });
        postServiceMock.Setup(p => p.GetPostsByUserId(It.IsAny<Guid>(), It.IsAny<PageRequest>()))
            .ReturnsAsync(PageImpl<PostDto>.Empty());
        userServiceMock.Setup(u => u.GetUserInfo(It.IsAny<Guid>(), It.IsAny<bool>(), It.IsAny<bool>())).ReturnsAsync(
            new UserInformationDto { UserId = userId, Username = "testuser", FriendsAmount = 5, SpacesAmount = 3 });
    ...
    public async Task FollowThisUser_FollowsUser()
    {
        ...
        postServiceMock.Setup(p => p.GetPostsByUserId(It.IsAny<Guid>(), It.IsAny<PageRequest>()))
            .ReturnsAsync(PageImpl<PostDto>.Empty());
        userServiceMock.Setup(u => u.GetUserInfo(It.IsAny<Guid>(), It.IsAny<bool>(), It.IsAny<bool>()))
            .ReturnsAsync(new UserInformationDto { UserId = userId, Username = "testuser" });
