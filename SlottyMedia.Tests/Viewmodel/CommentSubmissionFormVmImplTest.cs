...
public class CommentSubmissionFormVmImplTests
{
    ...
    public void SetUp()
    {
        ...
        _commentSubmissionFormVmImpl = new CommentSubmissionFormVmImpl(_mockAuthService.Object, _mockCommentService.Object, _mockNavigationManager.Object);
    }
    ...
    public async Task SubmitForm_DisplaysErrorWhenNotAuthenticated()
    {
        ...
        _mockCommentService.Verify(s => s.InsertComment(It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<string>()), Times.Never);
    ...
    public async Task SubmitForm_DisplaysErrorWhenTextIsEmpty()
    {
        ...
        Assert.That(_commentSubmissionFormVmImpl.TextErrorMessage, Is.EqualTo("Must provide some text in order to submit comment"));
        _mockCommentService.Verify(s => s.InsertComment(It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<string>()), Times.Never);
        _mockNavigationManager.Verify(n => n.Refresh(It.IsAny<bool>()), Times.Never);
    }
    ...
    public async Task SubmitForm_DisplaysServerErrorOnException()
    {
        ...
        _mockAuthService.Setup(s => s.GetCurrentSession()).Returns(new Session { User = new User { Id = userId } });
        _commentSubmissionFormVmImpl.Text = "This is a comment";
        _mockCommentService.Setup(s => s.InsertComment(It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<string>())).ThrowsAsync(new Exception());

        await _commentSubmissionFormVmImpl.SubmitForm(postId);

        Assert.That(_commentSubmissionFormVmImpl.ServerErrorMessage, Is.EqualTo("An unknown error occurred. Try again later."));
        _mockNavigationManager.Verify(n => n.Refresh(It.IsAny<bool>()), Times.Never);
    }
}